...
if(!empty($_GET['action'])){
    CompraControladores::main($_GET['action']);

}
class CompraControladores
{

    static function main ($action)
    {
        if ($action == "create") {
            CompraControladores::create();
        }/* else if ($action == "editar") {
           CompraControladores::editar();
        } else if ($action == "buscarId") {
          CompraControladores::buscarId($_REQUEST['Id']);
        } else if ($action == "ActivarCompra") {
            CompraControladores::ActivarCompra();
        } else if ($action == "InactivarCompra") {
            CompraControladores::InactivarPersona();
        }else if ($action == "login"){
            CompraControladores::login();
        }else if($action == "cerrarSession"){
            CompraControladores::cerrarSession();
        }*/
    }
    ...
    static public function create()
    {
        try {
            $arrayCompra = array();
            $arrayCompra['N_Factura'] = $_POST['N_Factura'];
            $arrayCompra['Fecha_Factura'] = $_POST['Fecha_Factura'];
            $arrayCompra['Descripcion'] = $_POST['Descripcion'];
            $arrayCompra['Valor_Total'] = $_POST['Valor_Total'];
            $arrayCompra['Proveedor_Id'] = $_POST['Proveedor_Id'];

            if (!Compra::CompraRegistrada($arrayCompra['N_Factura'])) {
                $Compra = new Compra ($arrayCompra);
                $Compra->create();
                header("Location: ../../Vistas/Modules/Compra/index.php?respuesta=correcto");
            } else {
                header("Location: ../../Vistas/Modules/Compra/create.php?respuesta=error&mensaje=Compra ya registrada");
            }

        } catch (Exception $e) {
            header("Location: ../../Vistas/Modules/Compra/create.php?respuesta=error&mensaje=" . $e->getMessage());
        }
    }
}
