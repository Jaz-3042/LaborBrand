...
if(!empty($_GET['action'])){
    RepuestoControladores::main($_GET['action']);
}else{
    echo "No se encontro ninguna accion...";
}
class RepuestoControladores
{
    ...
    static function main($action)
    {
        if ($action == "create") {
            RepuestoControladores::create();
             ...
             RepuestoControladores::editar();
        } else if ($action == "buscarID") {
             RepuestoControladores::buscarID($_REQUEST['id']);
        } else if ($action == "ActivarRepuesto") {
             RepuestoControladores::ActivarRepuesto();
        } else if ($action == "InactivarUsuario") {
             RepuestoControladores::InactivarRepuesto();
        }else if ($action == "login"){
            RepuestoControladores::login();
        }else if($action == "cerrarSession"){
             RepuestoControladores::cerrarSession();
        }*/
    ...
    static public function create()
    {
        try {
            $arrayRepuesto = array();
            $arrayRepuesto['Nombre'] = $_POST['Nombre'];
            $arrayRepuesto['Tipo'] = $_POST['Tipo'];
            $arrayRepuesto['Descripcion'] = $_POST['Descripcion'];


            if (!Repuesto::RepuestoRegistrado($arrayRepuesto['Nombre'])) {
                $Repuesto = new Repuesto ($arrayRepuesto);
                $Repuesto->create();
                header("Location: ../../Vistas/Modules/repuesto/index.php?respuesta=correcto&action=create");
            } else {
                header("Location: ../../Vistas/Modules/repuesto/create.php?respuesta=error&mensaje=Repuesto ya registrado");
            }

        } catch (Exception $e) {
            header("Location: ../../Vistas/Modules/repuesto/create.php?respuesta=error&mensaje=" . $e->getMessage());
        }
    }
