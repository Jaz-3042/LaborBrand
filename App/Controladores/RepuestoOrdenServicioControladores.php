...
require('../Modelos/Repuesto.php');
use App\Modelos\Repuesto;

if(!empty($_GET['action'])){
    Compra_RepuestoControladores::main($_GET['action']);
}else{
    echo "No se encontro ninguna accion...";
}

class RepuestoOrdenServicioControladores
{
    static function main($action)
    {
        if ($action == "create") {
            RepuestoControladores::create();
        }/* else if ($action == "editar") {
           RepuestoOrdenServicioControladores::editar();
        } else if ($action == "buscarID") {
            RepuestoOrdenServicioControladores::buscarID($_REQUEST['id']);
        } else if ($action == "ActivarRepuestoOrdenServicio") {
            RepuestoOrdenServicioControladores::ActivarRepuestoOrdenServicio();
        } else if ($action == "InactivarUsuario") {
           RepuestoOrdenServicioControladores::InactivarRepuestoOrdenServicio();
        }else if ($action == "login"){
            RepuestoOrdenServicioControladores::login();
        }else if($action == "cerrarSession"){
            RepuestoOrdenServicioControladores::cerrarSession();
        }*/
}
    static public function create()
    {
        try {
            $arrayRepuesto_Orden_Servicio = array();
            $arrayRepuesto_Orden_Servicio['Id'] = $_POST['Id'];
            $arrayRepuesto_Orden_Servicio['Costo_Venta'] = $_POST['Costo_Venta'];



            if (!Repuesto_Orden_Servicio::Repuesto_Orden_ServicioRegistrado($arrayRepuesto_Orden_Servicio['Id'])) {
                $Repuesto_Orden_Servicio = new Repuesto_Orden_Servicio ($arrayRepuesto_Orden_Servicio);
                $Repuesto_Orden_Servicio->create();
                header("Location: ../../Vistas/Modules/Repuesto_Orden_Servicio/index.php?respuesta=correcto&action=create");
            } else {
                header("Location: ../../Vistas/Modules/Repuesto_Orden_Servicio/create.php?respuesta=error&mensaje=Repuesto_Orden_Servicio ya registrado");
            }

        } catch (Exception $e) {
            header("Location: ../../Vistas/Modules/Repuesto_Orden_Servicio/create.php?respuesta=error&mensaje=" . $e->getMessage());
        }
    }
