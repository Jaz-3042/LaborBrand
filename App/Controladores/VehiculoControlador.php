...
if(!empty($_GET['action'])){
    VehiculoControlador::main($_GET['action']);
}else{
...
class VehiculoControlador
{
    ...
    {
        if ($action == "create") {
            \App\Controladores\ VehiculoControlador::create();
        }/* else if ($action == "editar") {
            VehiculoControlador::editar();
        } else if ($action == "buscarID") {
           VehiculoControlador::buscarID($_REQUEST['id']);
        } else if ($action == "ActivarN_Placa") {
            VehiculoControlador::ActivarColor();
        } else if ($action == "InactivarN_Placa") {
           VehiculoControlador::InactivarColor();
        }else if ($action == "login"){
            VehiculoControlador::login();
        }else if($action == "cerrarSession"){
            VehiculoControlador::cerrarSession();
        }*/
  ...
  static public function create()
{
    try {
        $arrayVehiculo= array();
        $arrayVehiculo['N_Placa'] = $_POST['N_Placa'];
        $arrayUsuario['Color'] = $_POST['Color'];

        if(!Vehiculo::VehiculoRegistrado($arrayVehiculo['N_Placa'])){
            $Vehiculo = new Vehiculo ($arrayVehiculo);
            $Vehiculo->create();
            header("Location: ../../Vistas/modules/vehiculo/index.php?respuesta=correcto&action=create");
        }else{
            header("Location: ../../Vistas/modules/vehiculo/create.php?respuesta=error&mensaje=Vehiculo ya registrado");
        }
    } catch (Exception $e) {
        header("Location: ../../Vistas/modules/vehiculo/create.php?respuesta=error&mensaje=" . $e->getMessage());
    }
}
